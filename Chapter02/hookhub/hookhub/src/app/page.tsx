...
import HeroSplit from "@/components/heros/HeroSplit";
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]">
      ...
      {/* Hero Variant 3 - Split Screen Style */}
      <div className="relative">
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-10 bg-black/60 text-white text-xs font-mono px-4 py-1.5 rounded-full border border-white/20 backdrop-blur-sm">
          ✦ Variant: HeroSplit — Split Screen Style
        </div>
        <HeroSplit />
      </div>
        ...
        <div className="max-w-6xl mx-auto px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between gap-6">
            ...
            <div className="flex-1 max-w-md">
              <div className="relative">
                <svg
                  className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--slate-light)]"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
          ...
          <section className="pb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {featuredHooks.map((hook, index) => (
                <div
                  key={hook.id}
                  className={`animate-fade-in-up opacity-0 stagger-${Math.min(index + 4, 6)}`}
                >
                  <HookCard hook={hook} />
                </div>
              ))}
