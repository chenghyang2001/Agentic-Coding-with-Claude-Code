/**
 * PM2 Ecosystem 設定檔 — HookHub Ch10（Next.js）
 *
 * 使用 __dirname 動態推算 cwd，確保跨機器可移植（不硬編碼 /home/<user>/）。
 * 在 VPS 執行：pm2 start ecosystem.config.js --env production
 *
 * Chapter10 獨立 PM2 app name 為 "hookhub-ch10"，
 * PORT 3002，與 Chapter02 的 "hookhub"（PORT 3001）隔離，避免衝突。
 */
...
module.exports = {
  apps: [
    {
      // 應用名稱；Chapter10 加 "-ch10" 後綴，與 Chapter02 的 "hookhub" 區分
      ...
      env_production: {
        NODE_ENV: "production",
        // PORT 以字串型別傳入，避免 PM2 將 Number 3002 傳給 Next.js 時發生隱式轉換歧義
        // Chapter02 用 3001，Chapter10 用 3002，避免同台 VPS 上 port 衝突
        PORT: "3002",
        // 強制 Next.js 只監聽 loopback（127.0.0.1），確保外部流量一定經過 nginx
        // 若不設此項，Next.js 預設監聽 0.0.0.0，port 3002 對外裸露可繞過 nginx 安全標頭
        HOSTNAME: "127.0.0.1",
      },
      ...
      // 加 -ch10 後綴確保不與 Chapter02 的 log 檔混淆
      out_file: path.join(__dirname, "logs", "hookhub-ch10-out.log"),
      error_file: path.join(__dirname, "logs", "hookhub-ch10-err.log"),
